...
enum class PDB_Debug_type { GDB, LLDB };

template <typename... Args>
std::unique_ptr<PDBDebugger> createDebugger(PDB_Debug_type type, Args... args) {
  if (type == PDB_Debug_type::GDB)
    return std::unique_ptr<PDBDebugger>(
        new GDBDebugger(std::forward<Args>(args)...));
  else if (type == PDB_Debug_type::LLDB)
    return std::unique_ptr<PDBDebugger>(
        new LLDBDebugger(std::forward<Args>(args)...));
 ...
 */
template <typename... DebuggerArgs>
void PDBDebug::launch(std::string start_rountine, std::string exec,
                      std::string args, PDB_Debug_type type,
                      DebuggerArgs... dargs) {
  ...
  for (int i = 0; i < proc_count; i++) {
    pdb_proc.emplace_back(
        createDebugger(type, std::forward<DebuggerArgs>(dargs)...));
