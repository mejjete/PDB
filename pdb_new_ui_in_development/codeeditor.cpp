...
// {
//     // Allow setting a breakpoint ONLY by clicking on numbers (number zone).
//     // In the gutter, the last ~18px are reserved for the circle and arrow.
//     const int w = width();
//     const int numbersRightEdge = w - 18; // Right border of the digit area.
//     if (event->position().x() <= numbersRightEdge) {
...
void LineNumberArea::mousePressEvent(QMouseEvent *event) {
  ...
  codeEditor->toggleBreakpointAtGutterY(
  ...
  while (block.isValid() && top <= event->rect().bottom()) {
    if (block.isVisible() && bottom >= event->rect().top()) {
      ...
      if (breakpoints.contains(blockNumber)) {
        ...
        painter.drawEllipse(QPoint(w - 9, midY), 6, 6);
      }

      // Arrow of the current line - also by midY
      if (blockNumber == execLine) {
        const QPoint pts[3] = {QPoint(w - 18, midY - 6),
                               QPoint(w - 18, midY + 6), QPoint(w - 2, midY)};
...
void CodeEditor::startDebug() {
  ...
  execLine = (nextStop < 0) ? qMin(execLine, blockCount() - 1) : nextStop;

  ensureLineVisible(execLine);
  lineNumberArea->update();
  update(); // Guarantee redrawing of the main viewport.
}
